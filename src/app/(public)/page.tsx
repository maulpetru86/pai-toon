...
import { fetchPublishedComics, fetchCategories, fetchCategoryById } from "@/lib/firebase/firestore";
...
export default function HomePage() {
  ...
  const featuredComic = comics.length > 0
    ? comics.reduce((a, b) => a.totalReaders > b.totalReaders ? a : b)
    : null;

  const latestComics = comics.slice(0, 6);
    ...
    <>
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                {featuredComic && (
                  <Link href={`/komik/${featuredComic.slug}`}>
                    <Button variant="outline" size="lg" className="gap-2 px-8 w-full sm:w-auto">
            ...
            {featuredComic && (
              <div className="hidden md:flex justify-center">
                <Link href={`/komik/${featuredComic.slug}`} className="group relative">
                  <div className="relative w-64 lg:w-72 rounded-2xl overflow-hidden shadow-2xl shadow-primary/20 ring-1 ring-border/40 transition-transform duration-500 group-hover:scale-[1.03] group-hover:shadow-primary/30">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 space-y-1">
                      ...
                      <div className="flex items-center gap-2 text-white/70 text-xs">
                        ...
                        <span>•</span>
                        <span>{featuredComic.totalReaders.toLocaleString()} pembaca</span>
                      </div>
                    </div>
                  </div>
                </Link>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="text-center mb-8">
          <h2 className="text-2xl md:text-3xl font-bold mb-2">Kategori Materi</h2>
          <p className="text-muted-foreground text-sm">Pilih materi PAI yang ingin kamu pelajari</p>
        </div>
          ...
          {categories.map((kat) => (
            <Link key={kat.id} href={`/komik?kategori=${kat.slug}`}>
              <div className="group flex flex-col items-center gap-3 p-5 md:p-6 rounded-xl border border-border/40 bg-card hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-1">
                ...
                <span className="text-xs md:text-sm font-medium text-center">{kat.name}</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="container mx-auto px-4 pb-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold mb-1">Komik Terbaru</h2>
            <p className="text-muted-foreground text-sm">Koleksi komik PAI terbaru untuk kamu</p>
          </div>
          <Link href="/komik">
      ...
      <section className="border-t border-border/40 bg-muted/20">
        <div className="container mx-auto px-4 py-16">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-bold mb-2">Kenapa PAI-Toon?</h2>
            <p className="text-muted-foreground text-sm">Cara baru belajar PAI yang disukai Gen Z</p>
          </div>

          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: "📱", title: "Baca Kapan Saja", desc: "Akses gratis tanpa login. Langsung scroll dan baca komik PAI di HP-mu." },
              { icon: "🎨", title: "Visual & Menarik", desc: "Materi PAI disajikan dalam bentuk komik vertikal bergaya Webtoon yang keren." },
              { icon: "🏆", title: "Gamifikasi Seru", desc: "Kumpulkan XP, naikkan level, dan pertahankan streak baca harianmu!" },
            ].map((feature) => (
              <div
                key={feature.title}
