...
export default function ReaderPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const comicData = await fetchComicBySlug(comicSlug);
        if (!comicData) { setLoading(false); return; }
        setComic(comicData);

        const [chapterData, chaptersData] = await Promise.all([
          fetchChapterByNumber(comicData.id, chapterNum),
  ...
  const currentIndex = publishedChapters.findIndex((ch) => ch.chapterNumber === chapterNum);
      ...
      <div className={`fixed top-0 left-0 right-0 z-50 transition-transform duration-300 ${showTopBar ? "translate-y-0" : "-translate-y-full"}`}>
        <div className="bg-black/90 backdrop-blur-md border-b border-white/10">
          <div className="container mx-auto px-4 flex items-center justify-between h-12">
            <Link href={`/komik/${comic.slug}`} className="flex items-center gap-2 text-white/70 hover:text-white transition-colors text-sm">
              <ArrowLeft className="h-4 w-4" />
              <span className="hidden sm:inline truncate max-w-[200px]">{comic.title}</span>
            </Link>
            <div className="text-center">
              <p className="text-white text-xs font-medium">Ch. {chapter.chapterNumber} — {chapter.title}</p>
              <p className="text-white/40 text-[10px]">{chapter.pages.length} halaman</p>
            </div>
            <Link href={`/komik/${comic.slug}`} className="text-white/70 hover:text-white transition-colors">
              ...
              {!loadedImages.has(index) && (
                <div className="w-full aspect-[2/3] bg-zinc-900 flex items-center justify-center">
                  <div className="text-center space-y-3">
                    ...
                    <p className="text-zinc-600 text-xs">Memuat halaman {index + 1}...</p>
                  </div>
                </div>
              )}
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="max-w-xl mx-auto">
            <div className="flex items-center gap-3 justify-center mb-6">
              ...
              <p className="text-white/40 text-xs whitespace-nowrap">Selesai — Ch. {chapter.chapterNumber}</p>
              <div className="h-px flex-1 bg-white/10" />
            </div>
            <div className="flex items-center gap-3">
              {prevChapter ? (
                <Link href={`/baca/${comic.slug}/${prevChapter.chapterNumber}`} className="flex-1">
                  <Button variant="outline" className="w-full gap-2 border-white/20 !bg-transparent !text-white hover:!bg-white/10">
                    <ChevronLeft className="h-4 w-4" />Ch. {prevChapter.chapterNumber}
                  </Button>
                </Link>
              ) : (<div className="flex-1" />)}
              <Link href={`/komik/${comic.slug}`}>
                <Button variant="outline" size="icon" className="border-white/20 !bg-transparent !text-white hover:!bg-white/10">
                  <List className="h-4 w-4" />
                </Button>
              </Link>
              {nextChapter ? (
                <Link href={`/baca/${comic.slug}/${nextChapter.chapterNumber}`} className="flex-1">
                  <Button className="w-full gap-2 bg-saffron text-black hover:bg-saffron/90 font-semibold">
                    Ch. {nextChapter.chapterNumber}<ChevronRight className="h-4 w-4" />
                  </Button>
                </Link>
              ) : (
                <div className="flex-1">
                  <Link href={`/komik/${comic.slug}`}>
                    <Button className="w-full gap-2">Kembali ke Daftar<ArrowLeft className="h-4 w-4" /></Button>
                  </Link>
                </div>
              )}
            </div>
